...
class RSOM(SOM):
    ...
    def find_bmu(self, x):
        ...
        # compute input distances
        diff_input = W_flat - x[np.newaxis, :]
        d_input = np.sum(diff_input**2, axis=1)
        # compute context distances
        diff_context = self.context_weights - self.context_vector[np.newaxis, :]
        d_context = np.sum(diff_context**2, axis=1)
        # combine distances
    ...
    def train(self, data, num_epochs: int = 100, mode: str = 'online'):
        ...
        for epoch in range(num_epochs):
            ...
            for x in data[perm]:
                # updateContext uses the previous state to compute new ct
