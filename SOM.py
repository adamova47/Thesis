...
class SOM:
    def __init__(
        ...
        *,
        weight_init_method: Union[
            str, callable
        ] = "uniform",  # string naming a built-in method or callable function
    ...
    def train(self, data, num_epochs: int = 100, mode: str = "online"):
        ...
        for epoch in range(num_epochs):
            ...
            if mode == "online":
                ...
                # quantization error
                dists = [
                    np.linalg.norm(x - self.weights[self.find_bmu(x)]) for x in data
                ]
                self.q_error_history.append(np.mean(dists))

                # average adjustment
                delta = self.weights - old_weights
                self.avg_adjust_history.append(
                    np.mean(np.linalg.norm(delta.reshape(-1, self.dim), axis=1))
                )
